...
def test_update_order_delivery_status_success(mocked_repo, order_service, test_orders_available):
...
def test_update_order_delivery_status_nonexistent_order(mocked_repo, order_service, test_orders):
    """Scenario: A nonexistent order should raise an error
    Input: A invalid order ID and a valid status
    Expected Behaviour: HTTPException 404 is raised"""
    mocked_repo.load_all_orders.return_value = test_orders
    mocked_repo.update_orders.return_value = None

    with pytest.raises(HTTPException) as exc_info:
        order_service.update_order_delivery_status("fake-id", "In_transit")

    assert exc_info.value.status_code == 404
    assert exc_info.value.detail == "Order fake-id Not Found."

def test_update_order_delivery_status_order_not_ready(mocked_repo, order_service, test_orders):
...
def test_update_order_delivery_status_invalid_status(mocked_repo, order_service, test_orders):
